...
export default function LoginForm({ redirectTo = "/" }) {
    ...
    const [isDoctorGoogleLoading, setIsDoctorGoogleLoading] = useState(false);
    ...
    return (
        <div className="flex min-h-[80vh] items-center justify-center bg-[#f8f9ff] dark:bg-[#233143] py-10 px-4 font-manrope transition-colors duration-200">
            <Card className="w-full max-w-lg p-5 sm:p-7 rounded-[2.5rem] bg-white dark:bg-[#002b5c] shadow-[0_15px_40px_rgba(0,63,131,0.05)] border border-[#c2c6d3]/30 dark:border-white/5">
                ...
                <div className="flex flex-col sm:flex-row gap-3 w-full pb-5">
                    ...
                    <Button
                        ...
                        onClick={handleGoogleSignIn}
                        isLoading={isGoogleLoading}
                        isDisabled={isLoading || isGoogleLoading || isDoctorGoogleLoading}
                        className="flex-1 h-14 rounded-xl font-[700] text-[14px] bg-transparent hover:bg-zinc-50 dark:hover:bg-white/5 text-[#00234a] dark:text-[#fff] border border-[#c2c6d3] dark:border-white/10 transition-all"
                    >
                        {!isGoogleLoading && (
                    ...
                    <Button
                        ...
                        isLoading={isDoctorGoogleLoading}
                        isDisabled={isLoading || isGoogleLoading || isDoctorGoogleLoading}
                        className="flex-1 h-14 rounded-xl font-[700] text-[14px] bg-transparent hover:bg-zinc-50 dark:hover:bg-white/5 text-[#00234a] dark:text-[#fff] border border-[#c2c6d3] dark:border-white/10 transition-all"
                    >
                        {!isDoctorGoogleLoading && (
                            <svg className="mr-2 h-4 w-4 shrink-0" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                <path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" fill="#4285F4" />
                                <path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" fill="#34A853" />
                                <path d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.06H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.94l3.66-2.85z" fill="#FBBC05" />
