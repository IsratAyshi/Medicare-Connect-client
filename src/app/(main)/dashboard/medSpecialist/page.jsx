...
import { getDoctorDashboardStats, getLoggedInDoctorProfile } from '@/lib/api/doctors';
...
const MedSpecialistDashboardHomepage = async () => {
    ...
    const stats = await getDoctorDashboardStats(doctorProfile?._id);
    ...
    const medSpecialistStats = [
        ...
        },
        {
            ...
            value: (stats?.todayAppointments || 0).toLocaleString(),
            icon: Calendar
        },
        {
            title: "Reviews Received",
            value: (stats?.reviewsReceived || 0).toLocaleString(),
            icon: Star
        },
    ];
